...
<html>
	...
	<body>
		...
		<hr>
		<h1 style="text-align: center;">Laporan Registrasi</h1>
		<hr>
		<table width="100%">
			<thead>
				<tr>
			...
			<tbody>
				...
				<tr>
					...
					<td>{{ $r->no_reg }}</td>
					<td>{{ $r->pembayaran->no_pembayaran}}</td>
					<td>{{ $r->pembayaran->siswa->nis }}</td>
					<td>{{ $r->siswa->kelas->nama_kelas }}</td>
					<td>{{ $r->semester }}</td>
					<td align="right">Rp. {{ number_format($r->pembayaran->jenispembayaran->nominal) }}</td>
					<td align="right">Rp. {{ number_format($r->bayar) }}</td>
					<td align="right">Rp. {{ number_format($r->tunggakan) }}</td>
				</tr>
				@empty
				<tr>
